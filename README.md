...
## Описание языка
`Mython` - интерпретатор Python-подобного языка.
Он является динамически типизированным, поддерживает классы, наследование, условные операторы, операции с целыми числами, печать в поток вывода.
...
Как было сказано выше, `Mython` - это язык с динамической типизацией, поэтому операция присваивания имеет семантику не копирования значения в область памяти, а связывания имени переменной со значением. Как следствие, переменные только ссылаются на значения, а не содержат их копии.
```python
class Counter:
  def __init__():
    self.value = 0
...
# Сборка и установка
Для сборки программы необходим компилятор С++, поддерживающий стандарт не ниже С++17, CMake не ниже версии 3.21.
...
<путь к файлу с кодом> , <путь для файла с выводом>
```
